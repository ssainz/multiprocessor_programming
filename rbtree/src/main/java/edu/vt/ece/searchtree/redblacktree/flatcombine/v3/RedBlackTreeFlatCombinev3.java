...
public class RedBlackTreeFlatCombinev3 <Key extends Comparable<Key>, Value> implements SearchTree<Key, Value> {
    ...
    public final ThreadLocal<Backoff> backoff = new ThreadLocal<Backoff>()
    {
        protected Backoff initialValue() {
            return BackoffFactory.getBackoff("Fixed");
        }
    };
    ...
    int threadNumber = 0;
    RedBlackTreeFlatCombineWorkerv3 worker = null;
    AtomicBoolean timeToFinish = new AtomicBoolean(false);

    public RedBlackTreeFlatCombinev3(int threadNumber){
        ...
        worker = new RedBlackTreeFlatCombineWorkerv3(this);
    ...
    private void goToSleep(AtomicBoolean aboutToSleep) {


        Backoff backoff = this.backoff.get();
        backoff.reset();
        while (true) {
            if (aboutToSleep.get()) {

                try {
                    backoff.backoff(); // backoff on failure
